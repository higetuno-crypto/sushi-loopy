...
1周目: 実績10件・強化5件・新聞15件・成長する音楽・モバイルUIに加え、世界鮮度同期から崩壊、最後の一貫、クリア記録まで実装。Save v3。2周目のLOOP・真EDは今後の工程。
...
世界鮮度同期装置を1台購入すると、「つぎの一歩」が「世界の同期を実行する」に変わる。追加費用なしで崩壊へ進み、約28秒の演出後に「最後の一貫を握る」でクリア。途中の文章は「ここまでの観測記録」で読み返せる。

崩壊・クリア中は通常生産と購入が止まる。画面を閉じても崩壊は進まず、途中から再開する。クリア後は営業時間・累計SUSHI・手で握った回数・設備の記録が残る。自動リセットは行わない。音とアニメーションは既存の操作で切り替えられる。

[実装と検証の詳細](docs/FIRST_RUN_ENDING.md)。`node scripts/ending-check.mjs` で隔離したEdgeのPC/モバイル環境から実購入→崩壊→リロード→クリアを検証できる。プレイヤーの保存領域は使わない。
...
v1/v2を読み込むとv3へ移行。旧バージョンのMain Saveは初回更新前にバックアップする。v1の旧クリック数は未記録なので0から、施設実績は所持数から付く。Code/Checkpoint復元でOffline報酬は付けない。読めない保存データは自動上書きせず警告する。保存コードは暗号化ではない。
