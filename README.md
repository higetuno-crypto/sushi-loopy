...
1周目: 実績10件・強化5件・新聞15件・成長する音楽・モバイルUIに加え、同期装置3台の導入に伴う異常、Three.jsによる画面崩壊、クリア記録まで実装。Save v4。2周目のLOOP・真EDは今後の工程。
...
2台目の接続処理中も握る・ほかの設備を買う操作は続けられる。異常が大きくなってから3台目を購入すると、応答エラーを経て普段のお店の画面が断片になって落ちる。最後に残る寿司を握ると1周目クリア。結末の予告・章立て・説明文は表示しない。

同期状態と異常の進行は保存され、画面を閉じても演出時間は進まない。旧版のクリア記録も維持する。クリア画面の「同期装置の購入前から試す」は記録をバックアップし、同期装置の購入費用を戻して再体験できる。ほかの設備・実績は残る。

[実装と検証の詳細](docs/FIRST_RUN_ENDING.md)。`scripts/ending-check.mjs` はPC/モバイルで購入→確認2回→異常→3台目購入→WebGL崩壊→クリア→再体験を検証。`SUSHI_TEST_URL`でbuild previewや公開先にも実行でき、保存領域は隔離したテストcontextだけを使う。
...
- 再体験前のクリア記録: `sushi-loopy.before-sync-replay`
...
v1/v2/v3を読み込むとv4へ移行。旧バージョンのMain Saveは初回更新前にバックアップする。v1の旧クリック数は未記録なので0から、施設実績は所持数から付く。Code/Checkpoint復元でOffline報酬は付けない。読めない保存データは自動上書きせず警告する。保存コードは暗号化ではない。
