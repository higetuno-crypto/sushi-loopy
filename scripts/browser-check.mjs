...
try {
  ...
  await check('10 taps -> real facility purchase -> achievement and v2 checkpoint',async()=>{
    ...
    assert.equal(cp.craftsman.save.schemaVersion,2);assert.deepEqual(cp.craftsman.save.game.unlockedAchievementIds,['first_craftsman']);
  });
  ...
  await check('fresh browser v1 migration + offline production + v1 backup',async()=>{
    ...
    assert.equal(result.save.schemaVersion,2);assert.ok(result.save.game.sushi>=80);assert.equal(result.save.game.totalClicks,0);assert.deepEqual(result.backup,old);await isolated.close();
  });
